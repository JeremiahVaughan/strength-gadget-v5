...
var AppUrl = "http://strengthgadget:8080" + ApiPrefix
...
func (c *IntegrationTestClient) runIntegrationTests() {
	...
	e := c.SendNotification(notificationMessage)
	if e != nil {
...
func (c *IntegrationTestClient) registerNewUser() (string, error) {
	response, err := c.attemptWithEmptyUsername(EndpointRegister)
	if response == nil {
