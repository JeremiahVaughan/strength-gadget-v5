...
// 	if haveMainExercisesJustBeenCompleted(req.ProgressIndex) {
...
// 		}
// 	} else if hasWorkoutBeenCompleted(req.ProgressIndex) {
// 		err = updateUserExerciseData(ctx, user)
// 		if err != nil {
// 			return fmt.Errorf("error, updating exercise data for user due to workout completion: %s. Error: %v", user.Id, err)
// 		}
// 	}
...
// 	serializedProgressIndex, err := json.Marshal(req.ProgressIndex)
// 	if err != nil {
...
func updateUserExerciseData(ctx context.Context, user *User) error {
	...
	if len(exerciseUserData) == 0 {
		return fmt.Errorf("error, empty exerciseUserData was not expected. User: %d", user.Id)
	}
	...
	if err != nil {
		return fmt.Errorf("error, updating exercise user data in database for user: %d. Error: %v", user.Id, err)
	}
	return nil
}
...
func hasWorkoutBeenCompleted(index WorkoutProgressIndex) bool {
	return len(index)-1 == int(WorkoutPhaseCompleted)
}
...
func haveMainExercisesJustBeenCompleted(incrementedProgressIndex WorkoutProgressIndex) bool {
	return len(incrementedProgressIndex)-1 == int(WorkoutPhaseCoolDown) &&
